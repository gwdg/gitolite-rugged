...
module Gitolite
  class Config
    ...
    def add_repo(repo, _overwrite = false)
      raise ArgumentError,
            'Repo must be of type Gitolite::Config::Repo!' unless repo.instance_of? Gitolite::Config::Repo
    ...
    def add_group(group, _overwrite = false)
      raise ArgumentError,
            'Group must be of type Gitolite::Config::Group!' unless group.instance_of? Gitolite::Config::Group
    ...
    def build_repos
      ...
      @repos.sort.each do |_k, v|
        repo_lines.push(v.to_s)

        gwd = v.gitweb_description
        gitweb_descs.push(gwd) unless gwd.nil?
      end

      repo_lines.push(gitweb_descs.join("\n"))
